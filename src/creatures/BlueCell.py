...
class BlueCell:
    def __init__(self, idx, scene, environment):
        ...
        self.creature_item.setBrush(QColor(200, 220, 240, 80))  # Light, semi-transparent blue
        self.creature_item.setPen(QPen(QColor(100, 140, 190), 2))  # Darker blue border for "cell membrane"
        ...
        shadow.setColor(QColor(20, 130, 120, 170))  # Semi-transparent dark shadow
        ...
        nucleus.setBrush(QColor(150, 200, 220, 200))  # Lightly colored, opaque center
        nucleus.setPen(QPen(QColor(15, 18, 18, 150), 1))
        ...
        shadow2.setColor(QColor(0, 60, 150, 150))  # Semi-transparent dark shadow
    ...
    def leave_trail(self):
        ...
        self.trail_markers.append((trail_marker, 130))  # Add as (marker, opacity) for fading
    ...
    def update_trail(self):
        ...
        for i, (marker, opacity) in enumerate(self.trail_markers):
            ...
            if opacity <= 0:
                ...
                marker.setBrush(QColor(150, 200, 255, opacity))  # Update marker color with new opacity
    ...
    def handle_edge_behavior(self, left, top, right, bottom, thoughts_manager):
        ...
        if x <= left + edge_proximity or x >= right - edge_proximity or y <= top + edge_proximity or y >= bottom - edge_proximity:
            ...
            if not hasattr(self, "boundary_phase"):
                self.direction = (self.direction + np.pi) % (2 * np.pi)  # Reverse direction
    ...
    def distance(self, other_creature):
        """Use FMU-calculated position to compute distance."""
        return np.hypot(
            self.creature_item.pos().x() - other_creature.creature_item.pos().x(),
    ...
    def update_from_fmu(self, position_x, position_y, distance_to_target, is_at_target):
        """Update creature's position and state based on FMU-calculated values."""
        boundaries = self.environment.boundary.get_boundaries()
        self.is_at_target = is_at_target
        self.distance_to_target = distance_to_target

        # Clamp to boundaries
        clamped_x = max(boundaries[0], min(position_x, boundaries[2]))
        clamped_y = max(boundaries[1], min(position_y, boundaries[3]))

        self.creature_item.setPos(clamped_x, clamped_y)
        self.leave_trail()
        self.update_trail()
